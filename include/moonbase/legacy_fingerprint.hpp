...
// The device id this SDK computed before it adopted the cross-SDK fingerprint
// spec: a bare, unstamped SHA-256 over `moonbase-cpp:fingerprint:v1` material.
//
// FROZEN. Everything below is load-bearing for licenses already issued against
// it, and "fixing" any of it would break exactly the bindings this class exists
// to keep accepting. That includes the parts that are plainly wrong:
//
//   * The material is LF-*terminated*, where the spec joins lines with LF.
//   * Values get an ASCII trim only: no NFC, no non-ASCII filtering, no length
//     cap, no placeholder rejection, no duplicate-name detection.
//   * Linux reads board_serial (mode 0400, so the id depends on privilege),
//     otherwise bios_* (which change on a firmware update), plus lscpu (whose
//     labels are translated, so the id depends on LANG).
//   * Windows collects SMBIOS type 4, whose structure count tracks the vCPU
//     count, and builds the RSMB provider signature byte-reversed, so
//     GetSystemFirmwareTable almost certainly returned 0 and every Windows
//     device id fell through to the host-name branch in device_id().
//   * An empty parameter set silently hashes the host name, handing every
//     unidentifiable machine on a platform one shared id.
//
...
public:
        ...
        std::string_view platform = platform_tag())
    {
        ...
        return detail::sha256_hex(material);
    }
